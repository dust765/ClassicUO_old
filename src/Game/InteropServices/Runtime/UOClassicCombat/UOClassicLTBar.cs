...
namespace ClassicUO.Game.InteropServices.Runtime.UOClassicCombat
{
    ...
    internal class UOClassicCombatLTBar : Gump
    {
        ...
        private static readonly Texture2D LINE_COLOR_DRAW_ORANGE = SolidColorTextureCache.GetTexture(Color.DarkOrange);
        ...
        public override void Update(double totalMS, double frameMS)
        {
            ...
            if (entity != null)
            {
                ...
                {
                    _barsLT[0].LineColor = LINE_COLOR_DRAW_ORANGE;
                    _borderLT[0].LineColor = LINE_COLOR_DRAW_ORANGE;

                    if (_borderLT.Length >= 3)
                    {
                        _borderLT[1].LineColor = _borderLT[2].LineColor = _borderLT[3].LineColor = LINE_COLOR_DRAW_ORANGE;
                    }
                }
                else
                {
        ...
        protected override bool OnMouseDoubleClick(int x, int y, MouseButtonType button)
        {
            ...
            if (CanMove)
            {
                ProfileManager.CurrentProfile.UOClassicCombatBuffbar_Locked = false;
                CanMove = false;
                AcceptMouseInput = false;
            }
            else
            {
                ProfileManager.CurrentProfile.UOClassicCombatBuffbar_Locked = true;
                CanMove = true;
                AcceptMouseInput = false;
            }
